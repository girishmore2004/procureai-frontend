...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Platform-admin accounts (cross-company oversight) skip the regular
      // buyer dashboard entirely and land on their own dedicated view.
      // Everyone else's very first login goes to Settings to save the
      // company profile and review permissions before using the app.
      if (data.data.user.is_platform_admin) navigate('/platform');
      else if (data.data.first_login) navigate('/settings');
