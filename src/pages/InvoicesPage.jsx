...
import api, { toAbsoluteFileUrl } from '../api/client';
import { Table, EmptyState, StatusBadge, PageLoader, Alert, Modal, Field } from '../components/ui';
import { Upload, CheckCircle, GitMerge, AlertTriangle, Wallet, MessageSquare, Send, RefreshCw } from 'lucide-react';
...
export function InvoiceDetailPage() {
  ...
  // Buyer <-> vendor message thread for this invoice (e.g. querying a
  // mismatch or delivery/payment terms). Mirrors the same thread already
  // wired up on the Purchase Order detail page.
  const { data: messages = [], refetch: refetchMsgs } = useQuery({
    queryKey: ['invoice-messages', id],
    queryFn: () => api.get(`/invoices/${id}/messages`).then((r) => r.data.data),
  ...
  const sendMessage = async () => {
    ...
    setSendingMsg(true);
    try {
      await api.post(`/invoices/${id}/messages`, { body: msgBody.trim() });
      setMsgBody('');
      refetchMsgs();
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Message thread — buyer <-> vendor conversation scoped to this invoice
          (e.g. flagging a mismatch or confirming payment terms). */}
      <div className="card">
        ...
        <div className="space-y-3 mb-4 max-h-80 overflow-y-auto">
          {messages.length === 0 ? (
            <p className="text-sm text-gray-400 text-center py-4">
              No messages yet regarding this invoice.
            </p>
          ) : messages.map((m) => (
